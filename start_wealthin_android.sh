#!/bin/bash
# WealthIn Android Startup Script
# Starts: PostgreSQL (if needed), Serverpod Server, Python Sidecar, Android Emulator, Flutter Android App

set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
WEALTHIN_DIR="$SCRIPT_DIR/wealthin"
SERVER_DIR="$WEALTHIN_DIR/wealthin_server"
FLUTTER_DIR="$SCRIPT_DIR/frontend/wealthin_flutter"
SIDECAR_DIR="$SCRIPT_DIR/backend"

# Colors for output
RED='\033[0;31m'
GREEN='\033[0;32m'
YELLOW='\033[1;33m'
BLUE='\033[0;34m'
NC='\033[0m' # No Color

echo -e "${BLUE}╔═══════════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║          WealthIn Android Startup Script                  ║${NC}"
echo -e "${BLUE}╚═══════════════════════════════════════════════════════════╝${NC}"
echo ""

# Function to check if a port is in use
check_port() {
    if netstat -tuln 2>/dev/null | grep -q ":$1 " || ss -tuln 2>/dev/null | grep -q ":$1 "; then
        return 0
    else
        return 1
    fi
}

# Function to kill process on port
kill_port() {
    local port=$1
    local pid=$(lsof -t -i:$port 2>/dev/null || fuser $port/tcp 2>/dev/null | awk '{print $1}')
    if [ -n "$pid" ]; then
        echo -e "${YELLOW}Killing existing process on port $port (PID: $pid)${NC}"
        kill -9 $pid 2>/dev/null || true
        sleep 1
    fi
}

# Export API keys (same as start_wealthin.sh)
echo -e "${GREEN}[1/4] Setting up environment variables...${NC}"
export ZOHO_CLIENT_ID="${ZOHO_CLIENT_ID:-1000.S502C4RR4OX00EXMKPMKP246HJ9LYY}"
export ZOHO_CLIENT_SECRET="${ZOHO_CLIENT_SECRET:-267a55dc05912009bb6ee13aabe1ea4e00c303e94d}"
export ZOHO_REFRESH_TOKEN="${ZOHO_REFRESH_TOKEN:-1000.9d9d2a78dd2bab8c51eb351f9f6d979f.904b8b7a8543ec3281d18749911184fd}"
export ZOHO_PROJECT_ID="${ZOHO_PROJECT_ID:-24392000000011167}"
export ZOHO_CATALYST_ORG_ID="${ZOHO_CATALYST_ORG_ID:-60056122667}"
export SARVAM_API_KEY="${SARVAM_API_KEY:-sk_j78ytxdk_VT7tOOTGMvuZD43XDcLoQKpm}"
export OPENAI_API_KEY="${OPENAI_API_KEY:-}"

echo -e "  ✓ Environment variables configured"

# Start Python Sidecar
echo -e "${GREEN}[2/4] Starting Python Sidecar...${NC}"
if check_port 8000; then
    echo -e "${YELLOW}  Port 8000 in use, restarting sidecar...${NC}"
    kill_port 8000
fi

cd "$SIDECAR_DIR"

if [ ! -d "venv" ]; then
    echo -e "  Creating Python virtual environment..."
    python3 -m venv venv
fi

source venv/bin/activate
pip install -q -r requirements.txt 2>/dev/null

nohup python main.py > sidecar.log 2>&1 &
SIDECAR_PID=$!
echo $SIDECAR_PID > sidecar.pid
echo -e "  ✓ Python Sidecar started (PID: $SIDECAR_PID) on port 8000"

deactivate 2>/dev/null || true

# Wait for sidecar
sleep 2
if ! check_port 8000; then
    echo -e "${RED}  Warning: Sidecar may not have started correctly${NC}"
fi

# Launch Emulator
echo -e "${GREEN}[3/4] Launching Android Emulator...${NC}"
cd "$FLUTTER_DIR"
EMULATOR_NAME="Medium_Phone_API_36.1"

# Check if emulator is already running
if flutter devices | grep -q "android"; then
    echo -e "  ✓ Android device/emulator already connected"
else
    echo -e "  Launching emulator: $EMULATOR_NAME..."
    flutter emulators --launch "$EMULATOR_NAME" &
    
    # Wait for emulator to be ready
    echo -e "  Waiting for emulator to boot (this may take a minute)..."
    while ! flutter devices | grep -q "android"; do
        sleep 5
        echo -n "."
    done
    echo ""
    echo -e "  ✓ Emulator connected"
fi

# Run Flutter Android App
echo -e "${GREEN}[4/4] Running Flutter Android App...${NC}"
cd "$FLUTTER_DIR"

# Get the emulator device ID
DEVICE_ID=$(flutter devices | grep "emulator" | head -n 1 | awk -F "•" '{print $2}' | xargs)

if [ -z "$DEVICE_ID" ]; then
    echo -e "${RED}Error: No emulator found!${NC}"
    echo -e "Please check if the emulator launched correctly."
    exit 1
fi

echo -e "Using emulator ID: $DEVICE_ID"

echo -e "  Building and running Android app on $DEVICE_ID..."
flutter run -d "$DEVICE_ID" 
