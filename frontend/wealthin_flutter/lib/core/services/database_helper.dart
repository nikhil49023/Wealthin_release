...
class DatabaseHelper {
  ...
  Future<void> _onUpgrade(Database db, int oldVersion, int newVersion) async {
    ...
    if (oldVersion < 9) {
      // Add notes column to transactions (used by SMS parsing)
      try {
        await db.execute('ALTER TABLE transactions ADD COLUMN notes TEXT');
        debugPrint('Added notes column to transactions (v9)');
      } catch (e) {
        // Column may already exist if created fresh
        debugPrint('notes column may already exist: $e');
      }

      // Create app_settings table for data source preferences
      await db.execute('''
  ...
  Future<void> _onCreate(Database db, int version) async {
    ...
    // App Settings Table for data source preferences
    await db.execute('''
