...
class PdfReportService {
  ...
  Future<String> generateHealthReport({
    ...
    // — Typography —
    final fTitle = PdfStandardFont(
      PdfFontFamily.helvetica,
      24,
      style: PdfFontStyle.bold,
    );
    final fH1 = PdfStandardFont(
      PdfFontFamily.helvetica,
      16,
      style: PdfFontStyle.bold,
    );
    final fH2 = PdfStandardFont(
      PdfFontFamily.helvetica,
      13,
      style: PdfFontStyle.bold,
    );
    final fBody = PdfStandardFont(PdfFontFamily.helvetica, 10.5);
    final fBold = PdfStandardFont(
      PdfFontFamily.helvetica,
      10.5,
      style: PdfFontStyle.bold,
    );
    final fSmall = PdfStandardFont(PdfFontFamily.helvetica, 8.5);
    final fScore = PdfStandardFont(
      PdfFontFamily.helvetica,
      44,
      style: PdfFontStyle.bold,
    );
    final fGrade = PdfStandardFont(
      PdfFontFamily.helvetica,
      14,
      style: PdfFontStyle.bold,
    );
    ...
    // — Header —
    g.drawRectangle(
      brush: PdfSolidBrush(cDark),
      bounds: Rect.fromLTWH(0, 0, ps.width, 90),
    );
    g.drawRectangle(
      brush: PdfSolidBrush(cPrimary),
      bounds: Rect.fromLTWH(0, 87, ps.width, 3),
    );
    g.drawString(
    ...
    final pillars = [
      _Pillar(
        'Savings Rate',
        healthScore.breakdown['savings'] ?? 0,
        30,
        'Save 20%+ of income to boost this pillar',
      ),
      _Pillar(
        'Debt Management',
        healthScore.breakdown['debt'] ?? 0,
        25,
        'Keep total EMIs below 35% of income',
      ),
      _Pillar(
        'Emergency Fund',
        healthScore.breakdown['liquidity'] ?? 0,
        25,
        'Target: 6 months of expenses saved',
      ),
      _Pillar(
        'Goal Progress',
        healthScore.breakdown['investment'] ?? 0,
        20,
        'Consistent SIP/RD improves this',
      ),
    ];
    ...
    for (final p in pillars) {
      ...
      g.drawString(
        '${p.score.toStringAsFixed(0)}/${p.max.toStringAsFixed(0)}',
        fSmall,
        brush: PdfSolidBrush(barColor),
    ...
    if (categoryBreakdown != null && categoryBreakdown.isNotEmpty) {
      ...
      for (var e in sorted.take(8)) {
        ...
        g.drawRectangle(
          brush: PdfSolidBrush(_catColor(e.key)),
          bounds: Rect.fromLTWH(
            _m + 112,
            y + 3,
            bw * (pct / 100).clamp(0, 1),
            10,
          ),
        );
        g.drawString(
          '${fmt.format(e.value)} (${pct.toStringAsFixed(0)}%)',
          fSmall,
          brush: PdfSolidBrush(cSubtle),
    ...
    _pageHeader(
      ...
      'Transparent 4-Pillar Methodology',
      cDark,
      cPrimary,
      fSmall,
    );
    ...
    final pillarDetails = [
      {
        'name': 'Savings (30 pts)',
        'score': healthScore.breakdown['savings'] ?? 0,
        'max': 30.0,
        'how':
            'Your savings rate × 30. ${dashboardData != null ? "Your rate: ${dashboardData.savingsRate.toStringAsFixed(1)}%." : ""}',
        'improve':
            'Set up auto-transfer of 10% of salary to a savings account on payday. Even ₹500/month extra compounds significantly over a year.',
      },
      {
        'name': 'Debt (25 pts)',
        'score': healthScore.breakdown['debt'] ?? 0,
        'max': 25.0,
        'how':
            '(1 − Debt-to-Income ratio) × 25. Below 35% is healthy, below 20% is excellent.',
        'improve':
            'Pay off highest-interest debt first (credit cards → personal loans → home loans). Avoid new EMIs until existing ones drop below 30% of income.',
      },
      {
        'name': 'Liquidity (25 pts)',
        'score': healthScore.breakdown['liquidity'] ?? 0,
        'max': 25.0,
        'how':
            '(Emergency fund months ÷ 6) × 25. Goal: 6 months of essential expenses saved.',
        'improve':
            'Start a liquid mutual fund or high-yield savings account. Target ₹${dashboardData != null ? fmt.format(dashboardData.totalExpense * 6) : "X"} as your emergency buffer.',
      },
      {
        'name': 'Goals (20 pts)',
        'score': healthScore.breakdown['investment'] ?? 0,
        'max': 20.0,
        'how': '(Amount saved toward goals ÷ Goal target) × 20.',
        'improve':
            'Set specific, time-bound goals and invest via SIP or RD. Even ₹1,000/month SIP grows to ₹2.1L in 10 years at 12% returns.',
      },
    ];
    ...
    for (var pd in pillarDetails) {
      ...
      g.drawString(
        '${sc.toStringAsFixed(1)} / ${mx.toStringAsFixed(0)}',
        fBold,
      ...
      _drawRoundedRect(
        ...
        24,
        cPrimaryL,
        PdfColor(167, 243, 208),
      );
      g.drawString(
        '→ ${pd['improve']}',
        fSmall,
        brush: PdfSolidBrush(PdfColor(6, 95, 70)),
    ...
    for (var t in tiers) {
      final isCurrent =
          score >= (t['lo'] as double) && score <= (t['hi'] as double);
      if (isCurrent) {
        _drawRoundedRect(
          g,
          _m,
      ...
      g.drawRectangle(
        brush: PdfSolidBrush(t['c'] as PdfColor),
    ...
    if (healthScore.aiAnalysis != null && healthScore.aiAnalysis!.isNotEmpty) {
      ...
      _pageHeader(
        ...
        ps,
        'AI-Powered Analysis',
        'Personalized insights from WealthIn AI',
        cDark,
        cPrimary,
        fSmall,
      );
      ...
      for (var line in lines) {
        ...
        // — Markdown table rows —
          ...
          final cells = tr
              ...
              .map(
                (c) => c.trim().replaceAll(RegExp(r'\*\*([^*]+)\*\*'), r'\1'),
              )
              .toList();
          if (cells.isNotEmpty) {
            g.drawString(
  ...
  void _pageHeader(
    ...
    g.drawRectangle(
      brush: PdfSolidBrush(dark),
      bounds: Rect.fromLTWH(0, 0, ps.width, 62),
    );
    g.drawRectangle(
      brush: PdfSolidBrush(primary),
      bounds: Rect.fromLTWH(0, 59, ps.width, 3),
    );
    g.drawString(
      title,
  ...
  void _metricCard(
    ...
    _drawRoundedRect(
      ...
      w,
      h,
      PdfColor(249, 250, 251),
      PdfColor(226, 232, 240),
    );
    g.drawRectangle(
      brush: PdfSolidBrush(color),
      bounds: Rect.fromLTWH(x, y, 3, h),
    );
    g.drawString(
      label,
      PdfStandardFont(PdfFontFamily.helvetica, 8.5),
  ...
  void _drawRoundedRect(
    ...
    g.drawRectangle(
      brush: PdfSolidBrush(fill),
      bounds: Rect.fromLTWH(x, y, w, h),
    );
    g.drawRectangle(
      pen: PdfPen(border, width: 0.5),
      bounds: Rect.fromLTWH(x, y, w, h),
    );
  }
  ...
  void _footer(PdfGraphics g, Size ps, PdfFont font) {
    g.drawLine(
      ...
      Offset(_m, ps.height - 32),
      Offset(ps.width - _m, ps.height - 32),
    );
    g.drawString(
  ...
  String _gradeMessage(double s) {
    if (s >= 80)
      return 'Excellent! You\'re managing your finances well. Keep going!';
    if (s >= 65) return 'Good progress! A few tweaks can take you further.';
    if (s >= 45)
      return 'You\'re on the right track. Let\'s focus on key improvements.';
