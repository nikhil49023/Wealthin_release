...
enum DocumentType { bankStatement, receipt, ticket, invoice, unknown }
...
class NativePdfParser {
  ...
  static Future<List<Map<String, dynamic>>> parseStatement(
    String filePath,
  ) async {
    try {
  ...
  static DocumentType _detectDocumentType(String text) {
    ...
    debugPrint(
      '[NativePDF] Scores - Bank: $bankScore, Receipt: $receiptScore, Ticket: $ticketScore, Invoice: $invoiceScore',
    );
  ...
  static List<ParsedTransaction> _parseUPIStatement(
    List<String> lines,
    String source,
  ) {
    ...
    for (int i = 0; i < lines.length; i++) {
      ...
      if (dateMatch != null) {
        ...
        for (int j = i + 1; j < (i + 10).clamp(0, lines.length); j++) {
          ...
          if (txType == null) {
            if (nextLine.toUpperCase() == 'DEBIT' ||
                nextLine.toLowerCase().contains('debited')) {
              txType = 'expense';
            } else if (nextLine.toUpperCase() == 'CREDIT' ||
                nextLine.toLowerCase().contains('credited')) {
              txType = 'income';
            }
          }
  ...
  static List<ParsedTransaction> _parseTraditionalBankStatement(
    List<String> lines,
    String source,
  ) {
    ...
    for (final line in lines) {
      ...
      if (amountResult != null && lastDate != null) {
        ...
        if (txType == 'unknown') {
          ...
          if (lineLower.contains('cr') ||
              lineLower.contains('credit') ||
              lineLower.contains('deposit') ||
              lineLower.contains('received') ||
              lineLower.contains('salary') ||
              lineLower.contains('refund') ||
              line.contains('+')) {
            txType = 'income';
          } else {
            txType = 'expense';
        ...
        try {
          ...
          if (groups[0] != null && RegExp(r'^[a-zA-Z]').hasMatch(groups[0]!)) {
            ...
            year = groups[2]!;
              ...
              RegExp(r'^[a-zA-Z]').hasMatch(groups[1]!)) {
            // DD MMM YYYY format
  ...
  static Map<String, dynamic>? _extractTransactionAmount(String text) {
    ...
    if (RegExp(
      r'\bCr\b|\bCR\b|\bcredit\b',
      caseSensitive: false,
    ).hasMatch(text)) {
      txType = 'income';
    } else if (RegExp(
      r'\bDr\b|\bDR\b|\bdebit\b',
      caseSensitive: false,
    ).hasMatch(text)) {
      txType = 'expense';
    }
  ...
  static List<ParsedTransaction> _deduplicateTransactions(
    List<ParsedTransaction> transactions,
  ) {
    final seen = <String>{};
    ...
    for (final tx in transactions) {
      final descPrefix = tx.description.length > 15
