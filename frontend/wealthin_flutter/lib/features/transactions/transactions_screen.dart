...
import '../../core/services/notification_transaction_service.dart';
...
class _TransactionsScreenState extends State<TransactionsScreen> {
  ...
  void initState() {
    super.initState();
    _loadTransactions();
    _startNotificationListener(); // Start notification-based detection
  }

  /// Start listening for banking notifications in the background.
  /// Transactions from notifications are auto-persisted by the service.
  Future<void> _startNotificationListener() async {
    try {
      final notifService = NotificationTransactionService();

      if (!await _isNotificationParsingEnabled()) {
        debugPrint(
          '[TransactionsScreen] Notification detection disabled in settings',
        );
        return;
      }

      // Only start if the user has enabled listener access
      if (!await notifService.isListenerEnabled()) {
        debugPrint(
          '[TransactionsScreen] Notification listener not enabled, skipping',
        );
        return;
      }

      debugPrint('[TransactionsScreen] Starting notification listener...');
      await notifService.startListening();
    } catch (e) {
      debugPrint(
        '[TransactionsScreen] Error starting notification listener: $e',
      );
    }
  }

  Future<bool> _isNotificationParsingEnabled() async {
    try {
      final db = await DatabaseHelper().database;
      final settings = await db.query(
        'app_settings',
        where: 'key IN (?, ?)',
        whereArgs: ['notification_parsing_enabled', 'sms_parsing_enabled'],
      );

      String? notificationValue;
      String? legacySmsValue;
      for (final row in settings) {
        ...
        if (key == 'notification_parsing_enabled') {
          ...
          legacySmsValue = value;
        }
      }

      final resolvedValue = notificationValue ?? legacySmsValue;
      ...
      debugPrint(
        '[TransactionsScreen] Failed to read notification parsing setting: $e',
      );
      return true;
    }
  }
