...
class _SplashScreenState extends State<SplashScreen> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    _textAnimationTimer = Timer(const Duration(milliseconds: 700), () {
      if (!mounted) return;
      _textController.forward();
    });
    
    // Complete splash after full animation
    _completionTimer = Timer(const Duration(milliseconds: 2100), () {
      if (!mounted) return;
      widget.onComplete();
    });
  }
                ...
                return Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    boxShadow: [
                      ...
                      BoxShadow(
                        color: const Color(0xFF4B7BEC).withValues(
                    ...
                    child: Container(
                      decoration: BoxDecoration(
                        gradient: LinearGradient(
                          ...
                          colors: [
                            const Color(0xFF1E3A5F).withValues(alpha: 0.9), // Dark navy
                            const Color(0xFF0D1B2A).withValues(alpha: 0.95), // Deeper navy
                          ],
                        ),
                        border: Border.all(
    ...
    for (final orb in orbs) {
      ...
      final paint = Paint()
        ..shader = RadialGradient(
          colors: [
            orb.color.withValues(alpha: scaledOpacity),
